...
public class MinPooling extends Pooling {

    public MinPooling(int width, int height, int padWidth, int padHeight) {
    ...
    protected void distributeGradient(float[] deltaInput, float[] inputData,
        ...
        for (int row = 0; row < poolHeight; row++) {
            ...
            for (int col = 0; col < poolWidth; col++) {
                ...
                if (y >= 0 && y < inputShape[1] && x >= 0 && x < inputShape[2]) {
                    ...
                    if (inputData[offset] < min) {
                        min = inputData[offset];
                        minOffset = offset;
                    }
                }
            }
        ...
        if (minOffset >= 0) deltaInput[minOffset] += grad;
    }
}
