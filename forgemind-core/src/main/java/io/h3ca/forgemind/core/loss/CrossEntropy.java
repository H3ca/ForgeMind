...
@JsonTypeName("CrossEntropy")
public class CrossEntropy implements LossFunction {
    public static CrossEntropy instance;
    ...
    public static CrossEntropy get() {
        if (instance == null) instance = new CrossEntropy();
        return instance;
    }
    ...
    public float computeLoss(Tensor yBatch, Tensor predictionBatch) {
        ...
        for (int sampleId = 0; sampleId < numSamples; sampleId++) {
            ...
            for (int classId = 0; classId < numClasses; classId++) {
                ...
                loss -= labelsData[offset] * (float) Math.log(predictionsData[offset] + EPSILON);
            }
        }
    ...
    public Tensor computeDelta(Tensor yBatch, Tensor predictionBatch) {
        ...
        for (int sampleId = 0; sampleId < numSamples; sampleId++) {
            ...
            for (int classId = 0; classId < numClasses; classId++) {
                int offset = baseOffset + classId;
                delta[offset] = predictionsData[offset] - labelsData[offset];
            }
        }
